...
export default function Inicio() {
  ...
  return (
    <div className="page">
      ...
      {/* Próximo */}
      {proximo && (
        <div style={{ background:'var(--verde)', borderRadius:14, padding:'1rem 1.25rem', marginBottom:'1rem', display:'flex', alignItems:'center', gap:12 }}>
          <div style={{ fontSize:28 }}>📅</div>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:10, color:'rgba(255,255,255,0.6)', textTransform:'uppercase', letterSpacing:'0.08em' }}>Próximo · J{proximo.jornada} · {proximo.local===EQUIPO_NOMBRE?'Local':'Visitante'}</div>
            <div style={{ fontSize:17, fontWeight:700, color:'white' }}>vs. {proximo.local===EQUIPO_NOMBRE?proximo.visitante:proximo.local}</div>
            <div style={{ fontSize:12, color:'rgba(255,255,255,0.7)' }}>{fmt(proximo.fecha)} · {proximo.campo}</div>
          </div>
        </div>
      )}
